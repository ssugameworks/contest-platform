...
>(({ label, items }, ref) => {
  ...
  const renderedItems = items.map((item, index) => {
    ...
    if (isFlyout) {
      return (
        <NavigationMenuRoot
          key={item.key ?? index}
          value={`${groupId}:${String(item.key ?? index)}`}
        >
    ...
    return (
      ...
      >
        <SeedSideNavigation.ItemCollapsibleTrigger
          current={collapsed && hasCurrentChild}
        >
          {item.prefixIcon && (
            <SeedSideNavigation.ItemPrefixIcon svg={item.prefixIcon} />
          )}
  ...
  return (
    <SeedSideNavigation.Group ref={ref}>
      {label && (
        <SeedSideNavigation.GroupLabel>{label}</SeedSideNavigation.GroupLabel>
      )}
      {renderedItems}
...
export interface SideNavigationProviderProps
  extends SeedSideNavigation.ProviderProps {}
